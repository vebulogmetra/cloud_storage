...
from app import db, exceptions, resources
...
class FileModel(NamedTuple):
    ...
    def from_telegram_document(cls, document, **kwargs):
        ...
        return cls(
            file_unique_id=file_unique_id, file_id=file_id, file_name=file_name, **kwargs
        )
...
def save_telegram_document(document, user_id, category):
    ...
    """
    file = FileModel.from_telegram_document(document, owner_id=user_id, category=category)
    save_file(file)


def save_telegram_photo(photo, user_id, category):
    file = FileModel.from_telegram_photo(photo, owner_id=user_id, category=category)
    save_file(file)
...
def save_file(file):
    ...
    try:
        ...
        raise exceptions.FileAlreadyExists(resources.FILE_ALREADY_EXISTS)
...
def get_owned_files(user_id):
    ...
    if not values:
        raise exceptions.NoUserFiles(resources.NO_FILES_UPLOADED_YET)
...
def get_category_files(category, user_id):
    ...
    if not values:
        raise exceptions.NoCategoryFiles(resources.NO_FILES_UPLOADED_YET)
